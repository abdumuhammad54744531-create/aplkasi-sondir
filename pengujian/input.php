...
<div class="alert alert-info py-2"><i class="bi bi-info-circle me-2"></i>Isi <strong>Cw</strong> dan <strong>Tw</strong>. Jenis tanah ditentukan otomatis dengan <strong>diagram Robertson 12 zona</strong> dari <strong>qc</strong> dan <strong>FR</strong>; qc dikonversi ke MPa untuk klasifikasi. Untuk paste Excel gunakan urutan <strong>Kedalaman, Cw, Tw</strong>.</div>
...
<th><span class="column-name">qc SBT</span><small class="column-unit">MPa</small></th>
...
<th><span class="column-name">Zona SBT</span><small class="column-unit">Robertson</small></th>
...
<script>window.sondirConfig=<?=json_encode(['id'=>$id,'alatId'=>(int)$titik['alat_id'],'interval'=>(float)$titik['interval_kedalaman'],'fk'=>(float)$titik['fk'],'ft'=>(float)$titik['ft'],'apiArea'=>(float)$titik['api'],'ac'=>(float)$titik['ac'],'as'=>(float)$titik['ass'],'capacity'=>(float)$titik['kapasitas_maksimum'],'unit'=>$titik['satuan_kapasitas'],'soilClassification'=>sondir_soil_chart_config(),'csrf'=>csrf_token(),'api'=>url('api/simpan-hasil.php'),'metaApi'=>url('api/simpan-kepala.php'),'coordinateApi'=>url('api/simpan-koordinat.php'),'searchApi'=>url('api/cari-lokasi.php'),'rows'=>$hasil,'points'=>array_map(fn($tab)=>['id'=>(int)$tab['id'],'nomor'=>(int)$tab['nomor_urut'],'nama'=>$tab['nama_titik'],'latitude'=>$tab['latitude']!==null?(float)$tab['latitude']:null,'longitude'=>$tab['longitude']!==null?(float)$tab['longitude']:null],$tabTitik)],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)?>;</script>
